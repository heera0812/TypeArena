...
function ResultsContent() {
  ...
  useEffect(() => {
    ...
    // Define fetchResults inside effect so it captures the current competitionId
  ...
  // fetchResults is now defined inside the useEffect to avoid stale closures.
  // This standalone version is kept for the CSV export (uses current results state).
  const exportToCSV = () => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto min-h-[85vh] flex flex-col pt-10 pb-20 px-4 print:p-0 print:w-full">
      ...
      <div className="flex justify-center gap-6 mt-8 print:hidden">
        ...
        <Link href="/register" className="text-slate-500 hover:text-slate-900 text-sm flex items-center gap-1">
          ← Back to Register
        </Link>
      </div>
    </div>
