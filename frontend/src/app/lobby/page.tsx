...
function LobbyContent() {
  ...
  // Keep ref in sync with state so callbacks always read fresh value
  const setMyParticipantIdSynced = (id: string | null) => {
  ...
  // Ref always holds the latest participantId — prevents stale closure in handleFinish/timer
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      socketRef.current!.emit("competition:join", { competitionId });
    });
    ...
    socket.on("lobby:players", (playerList: any[]) => {
      ...
      if (!isSpectator) {
        ...
        if (me) setMyParticipantIdSynced(me.id);
      }
    });
    ...
    socket.on("game:progress-update", (playerList: any[]) => {
      ...
      if (!isSpectator) {
        ...
        if (me) {
          ...
          if (me.id && !myParticipantIdRef.current) setMyParticipantIdSynced(me.id);
        }
      }
    });
  ...
  // ── handleFinish — declared BEFORE startDurationTimer to avoid TDZ issues ──
  const handleFinish = useCallback(() => {
    ...
    // Use ref to get latest participantId — avoids stale closure bug
    const partId = myParticipantIdRef.current;

    if (partId) {
      socketRef.current?.emit("game:finish", { competitionId, participantId: partId });
    }
  ...
  }, [hasFinished, competitionId, router]);
  ...
  const handleReady = () => {
    ...
    if (me) socketRef.current?.emit("player:ready", { competitionId, participantId: me.id });
  };

  // Admin controls
  const handleStart = () => socketRef.current?.emit("admin:start", { competitionId });
  const handlePause = () => socketRef.current?.emit("admin:pause", { competitionId });
  const handleResume = () => socketRef.current?.emit("admin:resume", { competitionId });
  const handleStop = () => {
    if (confirm("End the competition for all players?")) {
      socketRef.current?.emit("admin:stop", { competitionId });
    }
  };
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (me || myParticipantIdRef.current) {
      ...
      <div className="w-full max-w-5xl space-y-6">
        ...
        <div className="w-full bg-white border border-slate-100 rounded-3xl p-6 shadow-xl shadow-blue-500/5">
          ...
          <div className="space-y-6">
            {players.length === 0 ? (
              <div className="text-slate-400 text-sm text-center py-6">Waiting for players to join...</div>
            ) : players.map((p: any) => (
